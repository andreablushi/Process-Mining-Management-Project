...
The max depth parameters prevent overfitting while allowing sufficient model complexity to capture the underlying patterns. 
Note that the model uses only 3 max features, suggesting that at longer prefix lengths, individual activities become more discriminative for prediction.
...
For $\textit{prefix\_length}=5$, the model achieves balanced results around the $65\%$ for all considered metrics. 
...
At $\textit{prefix\_length}=5$, the recommendation system achieves a balanced accuracy better than the classifier, with all metrics around $80\%$. 
...
This strong performance, suggests that the recommendation system can be reliably deployed to guide ongoing cases toward positive outcomes. 
